...
if version_info[0] < 3 or version_info[1] < 6:
    LOGS.info("Ən az python 3.6 olması lazımdı."
...
UPSTREAM_REPO_URL = os.environ.get("UPSTREAM_REPO_URL","https://github.com/brendsupport/brenduserbot.git")
CONSOLE_LOGGER_VERBOSE = sb(os.environ.get("CONSOLE_LOGGER_VERBOSE", "False"))
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Salam mən` @BrendUserbot`! Mən sahibimə (`@{me.username}`) Kömək etmək üçün burdayam, sənə kömək edə bilmərəm. Ama sən də bir Brenduserbot işlədə bilərsən; Kanala bax` @BrendUserbot')
            else:
...
Hesabınızı bota çevirə və istifadə edə bilərsiniz. Unutmayın, başqasının botunu idarə edə bilməzsiniz! Bütün quraşdırma detalları aşağıdakı GitHub adresindən izah olunur.""",
        ...
        async def komut(event):
            ...
            await event.edit(
                result,
                buttons=[custom.Button.inline("◀️ Geri", data=f"Məlumat[{sayfa}]({cmd})")],
                link_preview=False
            )
        ...
        LOGS.info(
            ...
            "Aktivləşdirmək və botunuzda satır içi rejimini aktivləşdirmək üçün bot işarəsini təyin edin."
    ...
    try:
        ...
        LOGS.info(
            "BOTLOG_CHATID mühit dəyişən etibarlı bir şəxs deyil. "
